...
Limited links is a feature that allows you to restrict the number of devices that can add a subscription (link) to the application. In your personal account on the `happ-proxy.com` website, you specify your subscription URL, set the maximum number of devices, and click "Generate link." The resulting link can be shared with a user, and you can be confident that the subscription can only be installed on the specified number of devices — exceeding the limit is impossible.
...
- **{provider_code}** — your provider code (in your profile)
- **{auth_key}** — authorization key (in your profile)
- **{install_limit}** — maximum number of devices
...
Add the received **install_code** to the link as the **InstallID** parameter.
